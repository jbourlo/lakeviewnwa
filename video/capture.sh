...
# Changlelog
# jbourlo jul 12 - https://trac.ffmpeg.org/wiki/Capture/ALSA persist audio across reboots
ffmpeg  \
	-y -v verbose \
	-f v4l2 -i /dev/video0 \
	-f alsa -i hw:CARD=AudioPCI,DEV=0 \
	-t $1 \
	-framerate 29.97 -video_size 720x480 \
	-c:a aac -strict -2 -q 0 \
	$(date +"%m%d%Y_%H_%M.mp4")
